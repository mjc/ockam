...
use crate::error::map_vault_error_err;
use crate::types::*;
...
pub extern "C" fn ockam_vault_sha256(
    ...
    let output =
        VAULTS.call_with_result(&mut err, context, |v| -> Result<ByteBuffer, ExternError> {
            let d = v.vault.sha256(input).map_err(map_vault_error_err)?;
...
pub extern "C" fn ockam_vault_secret_generate(
    ...
    let handle = VAULTS.call_with_result_mut(
        ...
        |v| -> Result<SecretKeyHandle, ExternError> {
            let ctx = v.vault.secret_generate(atts).map_err(map_vault_error_err)?;
            Ok(v.add_secret(ctx))
        },
    );
    if err.get_code().is_success() {
    ...
    *output_buffer_length = 0;
    ...
    let output =
        VAULTS.call_with_result_mut(&mut err, context, |v| -> Result<ByteBuffer, ExternError> {
            let ctx = v.map.get(&secret).unwrap(); // FIXME
            let key = v.vault.secret_export(&ctx).map_err(map_vault_error_err)?;
...
pub extern "C" fn ockam_vault_secret_attributes_get(
    ...
    let output = VAULTS.call_with_result_mut(
        ...
        |v| -> Result<FfiSecretAttributes, ExternError> {
            let ctx = v.map.get(&secret_handle).unwrap(); // FIXME
            let atts = v
                .vault
...
pub extern "C" fn ockam_vault_secret_destroy(context: u64, secret: u64) -> VaultError {
    ...
    VAULTS.call_with_result_mut(&mut err, context, |v| -> Result<(), ExternError> {
        let ctx = v.map.remove(&secret).unwrap(); // FIXME
        v.vault.secret_destroy(ctx).map_err(map_vault_error_err)?;
        Ok(())
    });
    if err.get_code().is_success() {
        ERROR_NONE
...
pub extern "C" fn ockam_vault_hkdf_sha256(
    ...
    let handles = VAULTS.call_with_result_mut(
        ...
        |v| -> Result<OckamSecretList, ExternError> {
            let salt_ctx = v.map.get(&salt).unwrap(); // FIXME
            let ikm_ctx = if input_key_material.is_null() {
                None
            } else {
                unsafe {
                    Some(
                        v.map.get(&*input_key_material).unwrap(), // FIXME
                    )
                }
            };
...
pub extern "C" fn ockam_vault_get_persistence_id(
    ...
    let handle = Handle::from_u64(context).unwrap();
    let output = VAULTS
        .get(handle, |v| -> Result<CString, ExternError> {
            let ctx = v.map.get(&secret).unwrap(); // FIXME
            let persistence_id = v
                .vault
                .get_persistence_id(&ctx)
                .map_err(map_vault_error_err)?;
            let persistence_id = CString::new(persistence_id).unwrap();
            Ok(persistence_id)
        })
        .unwrap();
...
pub extern "C" fn ockam_vault_get_persistent_secret(
    ...
    let persistence_id = unsafe { CStr::from_ptr(persistence_id) }.to_str().unwrap();
    let mut err = ExternError::success();
    let handle = VAULTS.call_with_result_mut(
        &mut err,
        context,
