...
impl SecretVault for FilesystemVault {
    ...
    fn secret_export(&mut self, context: &Box<dyn Secret>) -> Result<SecretKey, VaultFailError> {
        ...
        let context = self.map.get(&context.0).unwrap(); // FIXME
    ...
    fn secret_destroy(&mut self, context: Box<dyn Secret>) -> Result<(), VaultFailError> {
        let id = FilesystemVaultSecret::downcast_secret(&context).unwrap().0; // FIXME
        ...
        let context = FilesystemVaultSecret::downcast_secret(&context)?;
        let context = self.map.remove(&context.0).unwrap(); // FIXME
...
impl AsymmetricVault for FilesystemVault {
    ...
    fn ec_diffie_hellman(
        ...
        let context = FilesystemVaultSecret::downcast_secret(context).unwrap(); // FIXME
        let context = self.map.get(&context.0).unwrap(); // FIXME
...
impl SignerVault for FilesystemVault {
    fn sign(
        ...
        let context = FilesystemVaultSecret::downcast_secret(secret_key).unwrap(); // FIXME
        let context = self.map.get(&context.0).unwrap(); // FIXME
...
impl HashVault for FilesystemVault {
    ...
    fn hkdf_sha256(
        ...
        let salt_context = FilesystemVaultSecret::downcast_secret(salt).unwrap(); // FIXME
        let salt_context = self.map.get(&salt_context.0).unwrap(); // FIXME

        let ikm = match ikm {
            Some(secret) => {
                let context = FilesystemVaultSecret::downcast_secret(secret).unwrap(); // FIXME
                Some(self.map.get(&context.0).unwrap()) // FIXME
            }
            None => None,
        };
